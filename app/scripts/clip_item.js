...
(function(window) {
	 ...
	 * Set date
	 *
	 * @param {Integer} date - millis from epoch
	 */
	 ...
	 */
	ClipItem.prototype.save = function(callback) {
		callback = callback || function() {};

		if (app.Utils.isWhiteSpace(this.text)) {
			callback(ClipItem.ERROR_EMPTY_TEXT);
			return;
		}

		const value = ClipItem._getStorable(this);
		chrome.storage.local.set({[this.text]: value}, function() {
			let error = null;
	 ...
	 * @param {errorCallback} callback
	 */
	ClipItem.prototype.remove = function(callback) {
		callback = callback || function() {};

		chrome.storage.local.remove(this.text, function() {
			let error;
			if (chrome.runtime.lastError) {
				error = chrome.runtime.lastError.message;
			}
	 ...
	 * @param {errorCallback} callback
	 */
	ClipItem.remove = function(keys, callback) {
		callback = callback || function() {};
	 ...
	 */
	ClipItem.isEmpty = function(callback) {
		callback = callback || function() {};
	...
	ClipItem._deleteOlderThan = function(time, callback) {
		...
		ClipItem.loadAll(function(error, items) {
			...
			if (keys.length) {
				app.ClipItem.remove(keys, function() {
